...
from pmtrendviz.models.manager import SpacySklearnPipelineManager
from pmtrendviz.train.data import sample_training_data
from pmtrendviz.utils import get_es_client, get_models_dir


@pytest.mark.usefixtures('manage_pytest_index', 'install_en_core_sci_sm', 'manage_prediction_indices')
class TestSpacySklearnPipelineManager:
    sample_data: pd.DataFrame = pd.DataFrame()
    n_samples: int = 100
    index: str = 'pytest'
    model_name: str = 'pytest-model'
    model = 'ner_tfidf_truncatedsvd_kmeans'
    stop_words = 'english'
    max_df: float = 0.999
    min_df: float = 0.001
    ...
    def test_create_default_spacy_sklearn_pipeline_manager(self) -> None:
        manager = ModelFactory.create(
            ...
            spacy_disable=self.spacy_disable)

        assert manager is not None
        assert isinstance(manager.model, Pipeline)
        assert isinstance(manager.nlp, spacy.Language)
    ...
    def test_create_custom_spacy_sklearn_pipeline_manager(self) -> None:
        manager = ModelFactory.create(
            self.model,
            self.include_title,
            self.include_abstract,
        ...
        assert manager.nlp.meta['disabled'] == ['parser']
    ...
    def test_save(self) -> None:
        ...
        assert os.path.exists(os.path.join(get_models_dir(), self.model_name, 'spacy.nlp'))
        assert not os.path.exists(os.path.join(get_models_dir(), self.model_name, 'spacy.json'))

    def test_save_spacy_config_only(self) -> None:
        manager = ModelFactory.create(
            self.model,
            self.include_title,
            self.include_abstract,
        ...
        manager.fit(self.sample_data)
        manager.save(self.model_name, spacy_config_only=True)

        assert os.path.exists(os.path.join(get_models_dir(), self.model_name, 'model.joblib'))
        assert os.path.exists(os.path.join(get_models_dir(), self.model_name, 'config.json'))
        assert os.path.exists(os.path.join(get_models_dir(), self.model_name, 'cluster_names.csv'))
        assert not os.path.exists(os.path.join(get_models_dir(), self.model_name, 'spacy.nlp'))
        assert os.path.exists(os.path.join(get_models_dir(), self.model_name, 'spacy.json'))
    ...
    def test_load(self) -> None:
        ...
        loaded_manager = SpacySklearnPipelineManager.load(self.model_name)
        ...
        assert isinstance(loaded_manager, SpacySklearnPipelineManager)
        assert isinstance(loaded_manager.model, Pipeline)
        assert isinstance(loaded_manager.nlp, spacy.Language)
    ...
    def test_get_label_trend(self) -> None:
        ...
        manager.precompute_predictions(self.index, max_new_predictions=1_000, batch_size=100, es=es)
        predictions_index_name = manager._construct_predictions_index_name(self.index)
